...
class FacebookCrawler:
    ...
    def write_in_json(self):
        ...
        f = open('Алёна Попова.json', 'w', encoding="utf-8")
    ...
    ###################################################################
    # def get_facebook_images_url(self):
    #     name_image = []
    #     soup = BeautifulSoup(self.driver.page_source, 'html.parser')
    #     name = soup.find_all(class_='_2nlw _2nlv')
    #     for n in name:
    #         my_name = n.get_text()
    #         name_image.append(my_name)
    #         break
    #
    #     soup2 = BeautifulSoup(self.driver.page_source, 'html.parser')
    #     self.links = soup2.find_all(class_='_11kf img')  # this class saves a profile photo
    #     if self.links:
    #         for link in self.links:
    #             str_link = str(link['src'])
    #             name_image.append(str_link)
    #             break
    #     else:
    #         self.links = soup2.find_all(class_='silhouette _11kf img')  # если нет фотки профиля
    #         for link in self.links:
    #             str_link = str(link['src'])
    #             name_image.append(str_link)
    #             break
    #     return name_image
    #
    # def get_photo(self, link, folder_name):
    #     """
    ...
    #             url = link[1]
    #         else:
    #             url = 'https://ru.bitcoinwiki.org/upload/ru/images/f/f9/Facebook-blockchain.png'
    #
    #         img_name = name + '.png'
    ...
    while not crawler_instance.friends_names.empty():
        ...
        if name not in visited:
            ...
            try:
                ...
                # photo_url = crawler_instance.get_facebook_images_url()  # parse photo url
                # crawler_instance.get_photo(photo_url, 'photos')  # save photo
                # crawler_instance.get_photo(photo_url, 'photos_fa')  # save photo
...
def main():
    ...
    # photo_url = crawler.get_facebook_images_url()  # parse photo url
    # # crawler.get_photo(photo_url, 'photos')  # only MY photo
    # crawler.get_photo(photo_url, 'photos_fa')  # only MY photo
